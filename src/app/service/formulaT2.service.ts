...
export class FormulaT2Service {
  ...
  getFormulaA(index: number) {
    return FormulaT2Service.getFormulaNameA(String(index));
  }

  getFormulaB(index: number) {
    return FormulaT2Service.getFormulaNameB(String(index));
  }
    ...
    this.inputs.forEach(s => {
      ...
      if (this.inputs.indexOf(s) !== (this.inputs.length - 1)) {
        ...
        if (type % 2 == 0) {
          ...
          const valueResult1 = String(sumInput).replace('.', '').substring(2, 3);
          const valueResult2 = String(sumInput).replace('.', '').substring(3, 4);
      ...
      resultList.push(<ResultInputModel>{
        ...
        up: s.up,
        low: s.low,
        value: String(sumInput).replace('.', '').substring(0, 4),
        result: checking,
